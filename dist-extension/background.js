...
function makeErc20Action(functionName, to, args, token, callData) {
  const decimals = token?.decimals ?? 18;
  if (functionName === "approve") {
    const [spender, amount2] = args;
    const amt = parseAmount(amount2, decimals);
    ...
    return {
      ...
      params: [
        ...
        { name: "amount", value: amt.human, raw: amount2.toString() },
        { name: "token", value: token ? `${token.symbol} (${formatUnits(amount2, decimals)})` : "unknown ERC-20", raw: to }
      ],
      human: `Approve ${token?.symbol ?? "token"}`,
      description: amt.unlimited ? `Approve ${shortAddr(spender)} to spend an unlimited amount of ${token?.symbol ?? "this token"}${toIsSelf ? " (notice: you are the spender)" : ""}.` : `Approve ${shortAddr(spender)} spending of ${amt.human} ${token?.symbol ?? ""}`.trim(),
      token
    };
  }
  if (functionName === "transfer") {
    const [to22, amount2] = args;
    return {
      ok: true,
      kind: "erc20_transfer",
      selector: "0xa9059cbb",
      ...
      params: [
        { name: "to", value: to22 },
        { name: "value", value: formatUnits(amount2, decimals), raw: amount2.toString() }
      ],
      human: `Send ${token?.symbol ?? "token"}`,
      description: `Send ${formatUnits(amount2, decimals)} ${token?.symbol ?? ""} to ${shortAddr(to22)}`.trim(),
      token
    };
  }
  const [from15, to2, amount] = args;
  return {
    ok: true,
    ...
    value: 0n,
    params: [
      { name: "from", value: from15 },
      { name: "to", value: to2 },
      { name: "value", value: formatUnits(amount, decimals), raw: amount.toString() }
    ],
    human: `Pull ${token?.symbol ?? "token"} (transferFrom)`,
    description: `Move ${formatUnits(amount, decimals)} ${token?.symbol ?? ""} from ${shortAddr(from15)} to ${shortAddr(to2)} via the ${shortAddr(to)} contract.`,
    token
  };
}
function makeNativeTransfer(to, value) {
  return {
    ok: true,
    ...
    description: value === 0n ? `Call ${shortAddr(to)} with no native value.` : `Send ${Number(Number(formatUnits(value, 18)).toFixed(4))} USDC to ${shortAddr(to)}.`
  };
}
async function decodeTx(tx, options = {}) {
  ...
  if (!to) {
    ...
    return {
      ...
      description: isCreation ? "Transaction deploys a contract. Contract-creation calldata cannot be decoded as a call." : "No target, no data, no value.",
  ...
  let args;
  try {
    const decoded = decodeFunctionData({ abi: erc20Abi2, data });
    args = decoded.args;
  } catch (e) {
    return {
      ok: false,
  ...
  return makeErc20Action(functionName, to, args, token, { from: tx.from });
}

// src/core/decoder/token-meta.ts
function makeTokenMetaFetcher(reader) {
  return async (addr) => {
    if (reader.getCode) {
      const code = await reader.getCode(addr);
      if (!code || code === "0x") return null;
    }
    const call2 = async (fn) => {
      try {
        return await reader.readContract({ address: addr, abi: erc20Abi2, functionName: fn });
      } catch {
        return void 0;
      }
    };
    const [name, symbol, decimals] = await Promise.all([call2("name"), call2("symbol"), call2("decimals")]);
    if (name === void 0 && symbol === void 0) return null;
    return {
      name: String(name ?? ""),
...
// src/core/utils/address.ts
function isZeroAddress(addr) {
  return /^0x0+$/i.test(addr);
}

// src/core/risk/rules/index.ts
function finding(is) {
  return is;
}
var unlimitedApproval = {
  id: "unlimited-approval",
  async run(decoded, _ctx) {
...
var transferFromSigner = {
  id: "transfer-from-signer",
  async run(decoded, ctx) {
    if (decoded.kind !== "erc20_transfer_from") return [];
    const from15 = decoded.params.find((p) => p.name === "from")?.value ?? "";
    const to = decoded.params.find((p) => p.name === "to")?.value ?? "";
    const isSelf = ctx.from && from15.toLowerCase() === ctx.from.toLowerCase();
    if (!isSelf) return [];
    return finding([
      {
        id: this.id,
        ...
        reason: `This transaction pulls ${decoded.token?.symbol ?? "tokens"} out of your own account (from=${from15.slice(0, 7)}\u2026${from15.slice(-4)}) and sends them to ${to.slice(0, 7)}\u2026${to.slice(-4)}. You are acting as the operator of your own allowance.`,
        evidence: `transferFrom(from=${from15}, to=${to})`,
        limitation: "Agent and auto-spend flows routinely sign pull-from-self operations; verify the recipient."
      }
    ]);
  }
};
var unexpectedNetwork = {
  id: "unexpected-network",
  async run(decoded, ctx) {
    if (ctx.expectedChainId === void 0) return [];
    if (decoded.chainId === void 0) return [];
...
async function runRiskEngine(decoded, ctx) {
  const findings = [];
  if (!decoded.ok) {
    findings.push({
      id: "decode-failed",
...
// src/extension/ai/ask.ts
var SYSTEM_PROMPT = `You are "Arc Lens", a transaction comprehension helper for the Arc blockchain.
...
function shouldEnrich(settings, summary) {
  return (settings.aiProvider === "gemini" || settings.aiProvider === "openai" || settings.aiProvider === "anthropic") && !!settings.apiKey && summary.severity !== "safe" && summary.severity !== "unknown";
}
...
function metaFetcherFor(client) {
  const fetcher = makeTokenMetaFetcher({
    readContract: (p) => client.readContract({ ...p, args: [] }),
    getCode: (a) => client.getCode({ address: a })
  });
  return { fetcher, hasCode: async (a) => {
...
function newDecoded(decoded, requestChainId) {
  if (requestChainId !== void 0) {
    const dec = requestChainId;
...
async function analyzeTypedSign(payload) {
...
async function analyzeGenericSign(method) {
...
async function analyze(method, payload) {
  const settings = await getSettings();
  const uid2 = newUid();
  ...
  if (method === "wallet_switchEthereumChain") {
    const s = {
      ...
      kind: "switch_chain",
      severity: payload.targetChain === settings.expectedChainId ? "safe" : "warning",
      highlights: [],
      count: 0,
      findings: []
    };
  ...
  void ARC_CHAIN_ID;
  return { uid: uid2, human: method, kind: "unhandled", severity: "unknown", highlights: [], count: 0 };
}
function maybeEnrich(summary, sender) {
  const tabId = sender.tab?.id;
  if (tabId === void 0) return;
  void getSettings().then((settings) => {
    if (!shouldEnrich(settings, summary)) return;
    return askAi({ provider: settings.aiProvider, apiKey: settings.apiKey ?? "", summary }).then((r) => {
