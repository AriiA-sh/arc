...
import { activeClient, currentAnalysisChainId } from '../arc/client'
...
const tokenMetaFetcher = makeTokenMetaFetcher({
  readContract: (p) => activeClient().readContract({ ...p, args: [] }) as any,
  getCode: (a) => activeClient().getCode({ address: a }),
})

const hasCode = async (addr: string) => {
  if (!isAddress(addr)) return false
  const code = await activeClient().getCode({ address: addr as `0x${string}` })
  return !!code && code !== '0x'
}
...
/** Analyze a tx hash fetched from the active Arc network (testnet or mainnet). */
export async function analyzeHash(hash: string): Promise<Analysis> {
  const chainId = currentAnalysisChainId()
  const tx = await activeClient().getTransaction({ hash: hash as `0x${string}` })
  const decoded = await decodeTx({ to: tx.to, value: tx.value, input: tx.input, from: tx.from }, { fetchTokenMeta: tokenMetaFetcher })
  const findings = await runRiskEngine(decoded, { chainId, from: tx.from, hasCode })
...
export async function analyzeCalldata(rawCalldata: string, to?: string, valueRaw?: string): Promise<Analysis> {
  ...
  const findings = await runRiskEngine(decoded, { chainId, hasCode })
